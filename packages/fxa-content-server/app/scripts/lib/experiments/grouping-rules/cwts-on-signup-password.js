...
// TODO make rollout rate configurable from redis
...
class CWTSOnSignupPasswordGroupingRule extends BaseGroupingRule {
  ...
  choose(subject) {
    ...
    if (ROLLOUT_RATE === 1 || this.isTestEmail(subject.email)) {
      ...
      return 'control';
    } else if (this.bernoulliTrial(ROLLOUT_RATE, subject.uniqueUserId)) {
  ...
  _isValidSubject(subject) {
    return subject && subject.uniqueUserId && subject.service;
  }
}
